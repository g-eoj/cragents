...
from cragents import Anchor, Block, CRAgent, Free, Think, Tools, vllm_model_profile
...
guide = [Think([Anchor("I think "), Block(1, 2), Anchor("Therefore "), Free()]), Tools()]


async def test_default_agent_output():
    agent = CRAgent(model)
    await agent.constrain(guide)
    assert agent.model_settings == snapshot(
        {
            "extra_body": {
                "chat_template_kwargs": {"add_generation_prompt": False, "enable_thinking": False},
                "guided_grammar": """\
start: <think> NL "I think " block_1 "Therefore " FREE </think> <tool_call> tool_call </tool_call>
block_1: p_1{1,1}
p_1: s_1{1,2} NL NL
s_1[lazy]: /[^\\.\\n]+/ (".")
